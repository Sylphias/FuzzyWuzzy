...
class PyjfuzzThread(threading.Thread):
...
def generate_hue_inputs(input_list):
    ...
    config = PJFConfiguration(Namespace(json={"on": "True", "bri": 1, "hue": 2, "sat": 3}, nologo=True, level=6))
    fuzzer = PJFFactory(config)
    hue_inputs = fuzzer.fuzzed
    return hue_inputs
...
def spawn_multiple_instances(noOfThread):
    ...
    for threads in range(noOfThread):
        fuzz_threads += [PyjfuzzThread(counter)]
        counter +=1
